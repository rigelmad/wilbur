...
ros::Publisher drive_commands_left_pub;
ros::Publisher drive_commands_right_pub;
...
void publishDriveCommand(struct mycmd cmds)
{
  ...
  */
  ...
  std_msgs::Float32 driveCommandLeft;
  std_msgs::Float32 driveCommandRight;

  drive_cmd_left = reduceResolution(drive_cmd_left);
  drive_cmd_right = reduceResolution(drive_cmd_right);

  driveCommandLeft.data = drive_cmd_left;

  driveCommandRight.data = drive_cmd_right; // CHANGED BECAUSE RIGHT SIDE WAS DRIVING IN REVERSE

  if (driveCommandLeft.data == last_left && driveCommandRight.data == last_right) {
  ...
  drive_commands_left_pub.publish(driveCommandLeft);
  drive_commands_right_pub.publish(driveCommandRight);

  last_left = driveCommandLeft.data;
  last_right = driveCommandRight.data;
}
...
int main(int argc, char **argv)
{
  ...
  drive_commands_left_pub = n.advertise<std_msgs::Float32>("/lwheel_vtarget", 1000);
  drive_commands_right_pub = n.advertise<std_msgs::Float32>("/rwheel_vtarget", 1000);
